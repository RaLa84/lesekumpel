// Knoten: "Story-Elemente vorbereiten" — v3 (2026-07-05)
// v2 + KNOWN-MAIN-CHARACTER-Hinweis, wenn eine Avatar-Hauptfigur gesetzt ist
// (avatarCharacter aus "Daten vorbereiten"): die Extraktion muss GENAU diese
// Figur als role:"main" liefern; ihre Attribute überschreibt "Elemente parsen".
const d = $input.item.json;
let avatarChar = null;
try { avatarChar = ($('Daten vorbereiten').first().json || {}).avatarCharacter || null; } catch (e) {}
const knownMainSection = avatarChar ? `
KNOWN MAIN CHARACTER (user-provided):
- The story's main character is named "${avatarChar.name}" (type: ${avatarChar.type}).
- Extract this character with role "main" and this EXACT name. Do NOT assign role "main" to any other character.
- Its appearance will be replaced by a fixed user-provided reference afterwards, so a rough description is fine.
...
${knownMainSection}Story (paragraphs numbered 0-based; a blank line separates paragraphs — use these indices for appearsFromParagraph):
${(d.storyText || '').split(/\n\n+/).map((p, i) => `[${i}] ${p}`).join('\n\n')}`;
