// Knoten: "Szenen parsen" — Sachtext-Workflow (Samira), v1
// Kopie von szenen-parsen-v2.js (PROD-geteilt, dort NICHT ändern) + Auto-Infografik:
//   - Szenenfeld imageKind ('illustration' | 'diagram') aus dem Compiler-JSON übernehmen,
//     Max-1-Diagramm-Enforcement (erste gewinnt, weitere -> illustration)
//   - Diagramm-Szenen: DIAGRAM LOCK + Diagramm-Stiltexte statt VISUAL LOCK/User-Stil,
//     reduzierte Negatives (figuren-/timeline-bezogene Klauseln sind dort Rauschen)
//   - pro Szene styleRefUrl (diagram -> diagramStyleRefUrl) für den Gemini-Fallback
...
// Auto-Infografik: imageKind normalisieren — höchstens EINE Diagramm-Szene
// (die erste gewinnt, weitere werden zu illustration zurückgestuft)
let _diagramSeen = false;
const _sceneKinds = scenes.map(s => {
  const wantsDiagram = String((s && s.imageKind) || '').toLowerCase() === 'diagram';
  if (wantsDiagram && !_diagramSeen) { _diagramSeen = true; return 'diagram'; }
  return 'illustration';
});

return scenes.map((s, _sceneArrIdx) => {
  const sceneNo     = (typeof s.scene === 'number' && s.scene > 0) ? s.scene : 1;
  const imageKind   = _sceneKinds[_sceneArrIdx];
  const isDiagram   = imageKind === 'diagram';
  const diagramType = isDiagram ? (clean(s.diagramType) || 'cross-section') : '';
  ...
  const charsLine = chars.length > 0 ? chars.join(', ') : (isDiagram ? 'schematic elements only, no characters' : 'characters from VISUAL LOCK');
  ...
  // Finaler Prompt: FULL-BLEED Lead → Kontext → LOCK → SCENE → [MEDIUM] → STYLE → NEGATIVE
  // Diagramm-Szene: DIAGRAM LOCK + Diagramm-Stil statt VISUAL LOCK/User-Stil;
  // figuren-/timeline-bezogene Negatives entfallen dort (Rauschen für Schaubilder).
  const lockBlock = isDiagram ? (prev.diagramLock || visualLock) : visualLock;
  const contextLine = isDiagram
    ? "Clean schematic infographic panel for a published German children's non-fiction book, diagram type: " + diagramType + '.'
    : safetyContext + '.';
  const styleLine = 'ART STYLE: ' + (isDiagram ? (prev.diagramStylePositive || imageStylePositive) : imageStylePositive);
  ...
  const fullNegative = (isDiagram
    ? [prev.diagramStyleNegative || imageStyleNegative, frameNegative]
    : [imageStyleNegative, safetyNegative, frameNegative, dynamicNegative, timelineNegative]
  ).filter(has).join(', ');
  ...
  return {
    json: {
      ...
      imageKind,
      diagramType,
      // pro Szene: Diagramm-Szene bekommt im Gemini-Fallback die Schaubild-Referenz
      styleRefUrl: isDiagram ? (prev.diagramStyleRefUrl || prev.styleRefUrl) : prev.styleRefUrl,
