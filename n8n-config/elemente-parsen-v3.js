// Knoten: "Elemente parsen" — v3 (2026-07-05)
// v2 + deterministischer Override der role:'main'-Figur durch die
// Avatar-Hauptfigur (avatarCharacter aus "Daten vorbereiten"): feste,
// nutzerdefinierte Attribute schlagen die LLM-Extraktion, damit die
// Hauptfigur über alle Szenenbilder identisch aussieht (VISUAL LOCK).
...
// ── Avatar-Hauptfigur: Override VOR dem VISUAL-LOCK-Bau ──
let avatarChar = null;
...
if (avatarChar && avatarChar.name) {
  ...
  // Match per Name (Extraktion wurde per Prompt auf diesen Namen verpflichtet),
  // Fallback: erste role:'main'-Figur; sonst vorn einfügen.
  let idx = storyElements.characters.findIndex(c => nm(c.name) === nm(avatarChar.name));
  if (idx < 0) idx = storyElements.characters.findIndex(c => nm(c.role) === 'main');
  const merged = JSON.parse(JSON.stringify(avatarChar));
  merged.role = 'main';
  if (idx >= 0) storyElements.characters[idx] = merged;
  else storyElements.characters.unshift(merged);
