"""Phase 1.1 — Stil-Konsistenz, Aspect-Ratio, Outfit-Enforcement.

Behebt drei beobachtete Probleme:
1. Schwarze Balken / Letterbox (alle Stile betroffen)
2. Pixel-Auflösung variiert zwischen Bildern (widerspruechliche Stil-Beschreibung)
3. Nebenfiguren (z.B. Maja) tragen andere Outfit-Farben als in characterReference definiert

Änderungen:
- bildstilMap neu formuliert: klare Stil-DNA pro Entry, aspect ratio 1:1 enforcement
- Negatives erweitert: no letterboxing, no black bars, no borders, no aspect-ratio padding
- agent-szenen Systemprompt: Outfit-Enforcement für alle benannten Charaktere
"""
...
# ─────────────────────────────────────────────────────────────
# bildstilMap komplett neu formulieren in prepare-data
# ─────────────────────────────────────────────────────────────

NEW_BILDSTIL_BLOCK = '''const bildstilMap = {
  'Aquarell': {
    positive: "traditional children's book watercolor illustration, hand-painted on warm cream paper, soft pastel palette, gentle wet-on-wet washes with visible bleeding edges, loose expressive brush strokes, soft painterly rendering, inspired by Beatrix Potter and Quentin Blake, consistent palette and brush handling across all panels of this story, same character proportions and facial features in every panel, square 1:1 composition, full-frame edge-to-edge artwork",
    negative: "no text, no watermarks, no letterboxing, no black bars, no borders, no frames, no aspect-ratio padding, no hard black outlines, no digital vector look, no CGI, no 3D shading, no photorealism, no extra limbs, no duplicate props, no floating objects, no mixed art styles, no style drift"
  },
  'Cartoon': {
    ...
    negative: "no text, no watermarks, no letterboxing, no black bars, no borders, no frames, no aspect-ratio padding, no extra limbs, no duplicate props, no floating objects, no mixed art styles, no style drift, no gritty textures, no realistic shading, no photorealism"
  },
  'Buntstift': {
    positive: "colored pencil drawing on warm cream paper, visible pencil grain and diagonal hatching strokes, soft muted earthy palette, slightly sketchy outlines, hand-drawn children's illustration feel, consistent stroke density and palette across all panels of this story, same character proportions and facial features in every panel, square 1:1 composition, full-frame edge-to-edge artwork",
    negative: "no text, no watermarks, no letterboxing, no black bars, no borders, no frames, no aspect-ratio padding, no crayon texture, no digital smooth gradients, no vector look, no photorealism, no extra limbs, no duplicate props, no floating objects, no mixed art styles, no style drift"
  },
  'Pixel-Art': {
    positive: "modern 32-bit style pixel art in the aesthetic of Game Boy Advance JRPGs such as Advance Wars and Fire Emblem, detailed character sprites with clearly defined dark outlines, rich but limited 32-color palette with subtle dithering for shading, uniform pixel density across the entire image, consistent sprite resolution and palette across all panels of this story, same character sprite silhouette and facial features in every panel, 3/4 front view or side view composition, square 1:1 composition, full-frame edge-to-edge artwork",
    negative: "no text, no watermarks, no letterboxing, no black bars, no borders, no frames, no aspect-ratio padding, no mixed pixel resolutions between regions, no smooth gradients, no anti-aliasing, no blurry edges, no 3D shading, no photorealism, no high-res rendering, no extra limbs, no duplicate props, no floating objects, no style drift"
  },
  'Anime': {
    ...
    negative: "no text, no watermarks, no letterboxing, no black bars, no borders, no frames, no aspect-ratio padding, no extra limbs, no duplicate props, no floating objects, no mixed art styles, no style drift, no gritty shading, no sketchy outlines, no photorealism"
  },
  'Traumwelt': {
    ...
    negative: "no text, no watermarks, no letterboxing, no black bars, no borders, no frames, no aspect-ratio padding, no harsh black outlines, no flat cartoon shading, no photorealism, no gritty realism, no extra limbs, no duplicate props, no floating objects, no mixed art styles, no style drift"
  },
  'Knete': {
    ...
    negative: "no text, no watermarks, no letterboxing, no black bars, no borders, no frames, no aspect-ratio padding, no 2D flat shading, no painted illustration look, no CGI plastic sheen, no photorealism, no extra limbs, no duplicate props, no floating objects, no mixed art styles, no style drift"
  },
  'Voxel': {
    ...
    negative: "no text, no watermarks, no letterboxing, no black bars, no borders, no frames, no aspect-ratio padding, no smooth curved surfaces, no anti-aliasing, no photorealism, no detailed textures on voxels, no extra limbs, no duplicate props, no floating objects, no mixed art styles, no style drift"
  }
};'''
...
# ─────────────────────────────────────────────────────────────
# agent-szenen Systemprompt — Outfit-Enforcement ergänzen
# ─────────────────────────────────────────────────────────────
...
NEW_AGENT_SZENEN_SYS = (
    ...
    "ASPECT RULE: All scenes are square 1:1 full-frame compositions. Never describe borders, letterboxing, or black bars.\n\n"
...
# ─────────────────────────────────────────────────────────────
# prep-scenes — expliziter Outfit-Repeat im Slot-Template
# ─────────────────────────────────────────────────────────────

pc = nodes["prep-scenes"]["parameters"]["jsCode"]
OLD_SLOT = '"positive": "<ART_STYLE_POSITIVE>. Character: <CHAR_REF>. Ages: <explicit age for every human>. Composition: <camera angle>, <action verb>. Spatial: <ON|NEXT-TO|BEHIND|IN-FRONT-OF> <object>. Props in hands: left=<X or none>, right=<Y or none>. Setting: <location>. Mood: <adjective>.",'
NEW_SLOT = '"positive": "<ART_STYLE_POSITIVE>. Characters present (repeat full clothing description for each, verbatim from CHARACTER REFERENCE): <CHAR_REF_SUBSET>. Ages: <explicit age for every human>. Composition: <camera angle>, <action verb>. Spatial: <ON|NEXT-TO|BEHIND|IN-FRONT-OF> <object>. Props in hands: left=<X or none>, right=<Y or none>. Setting: <location>. Mood: <adjective>.",'
assert OLD_SLOT in pc, "Slot-positive line not found"
nodes["prep-scenes"]["parameters"]["jsCode"] = pc.replace(OLD_SLOT, NEW_SLOT)
