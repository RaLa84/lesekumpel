...
CSS_BLOCK = """
        ...
        /* Konzeptkarte "Das Wort heute" */
        .wdt-card { background: #2B3140; color: #FFF6EF; border-radius: 32px; padding: 44px 34px; text-align: center; position: relative; overflow: hidden; }
        ...
        .wdt-label { display: inline-block; background: #FFD95A; color: #2B3140; padding: 7px 16px; border-radius: 999px; font-family: 'Fredoka', sans-serif; font-weight: 600; font-size: 0.85em; margin-bottom: 20px; position: relative; z-index: 1; }
        .wdt-wort { font-family: 'Fredoka', sans-serif; font-weight: 700; font-size: 1.9em; line-height: 1.15; margin-bottom: 6px; position: relative; z-index: 1; }
        .wdt-wort .wdt-underline { background: linear-gradient(transparent 65%, #F97352 65%); padding: 0 6px; }
        .wdt-silben { font-family: 'Fredoka', sans-serif; font-size: 0.95em; opacity: 0.75; margin-bottom: 16px; position: relative; z-index: 1; }
        .wdt-bedeutung { font-size: 1em; line-height: 1.65; max-width: 560px; margin: 0 auto; opacity: 0.94; position: relative; z-index: 1; }
...
JS_BLOCK = """        window.imagePositions = imagePositions; // Für Tafel-Modus (Sticky-Bild)
        ...
        (function () {
            ...
            function wusstestHtml(text) {
                return '<div class="sachtext-box sachtext-box-wusstest"><button type="button" class="sachtext-box-head wusstest-toggle" aria-expanded="false">' + ICON_SPARKLES + '<span>Wusstest du?</span>' + ICON_CHEVRON + '</button><div class="sachtext-box-body">' + words(text) + '</div></div>';
            }
            ...
            function kurzfassungHtml() {
                ...
                return '<div class="sachtext-box sachtext-box-summary"><button type="button" class="sachtext-box-head wusstest-toggle" aria-expanded="false">' + ICON_BULB + '<span>Das Wichtigste in Kürze</span>' + ICON_CHEVRON + '</button><div class="sachtext-box-body">' + words(rawSummary) + '</div></div>';
            }
            function wortDesTagesHtml() {
                try {
                    const e = (typeof rawDictionaryEntry !== 'undefined' && Array.isArray(rawDictionaryEntry)) ? rawDictionaryEntry[0] : null;
                    if (!e || !e.wort || !e.bedeutung) return null;
                    const wort = String(e.wort).replace(/\\u00AD/g, '').replace(/-/g, '');
                    const silben = e.silben ? String(e.silben) : '';
                    return '<div class="wdt-card"><span class="wdt-label">Das Wort heute</span><div class="wdt-wort"><span class="wdt-underline">' + esc(wort) + '</span></div>' +
                        (silben ? '<div class="wdt-silben">' + esc(silben) + '</div>' : '') +
                        '<div class="wdt-bedeutung">' + words(e.bedeutung) + '</div></div>';
            ...
            window.computeStoryInserts = function (allGroups, ctx) {
                ...
                if (!ctx.summary) {
                    const wdt = wortDesTagesHtml();
                    if (wdt) {
                        const after = (K >= 2) ? gapEnd(Math.floor((K - 1) / 2)) : allGroups.length - 1;
                        inserts.push({ afterGroup: after, html: wdt, className: 'insert-wortdestages', indicatorLabel: 'Das Wort heute', asChunk: true });
                    }
                }
            ...
            window.decorateStoryView = function (view) {
                ...
                children.forEach(el => {
                    ...
                    const standalone = el.tagName === 'ASIDE' && (el.classList.contains('insert-lernblock') || el.classList.contains('insert-wortdestages') || el.classList.contains('insert-kurzfassung'));
                    if (standalone) {
...
def main() -> int:
    ...
    checks = {
        ...
        "Konzeptkarte": "insert-wortdestages" in out and "wdt-card" in out,
