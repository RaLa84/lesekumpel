...
JS_BLOCK = """        window.imagePositions = imagePositions; // Für Tafel-Modus (Sticky-Bild)
        ...
        (function () {
            ...
            function wusstestHtml(text) {
                return '<div class="sachtext-box sachtext-box-wusstest"><button type="button" class="sachtext-box-head wusstest-toggle" aria-expanded="false">' + ICON_BULB + '<span>Wusstest du?</span>' + ICON_CHEVRON + '</button><div class="sachtext-box-body">' + words(text) + '</div></div>';
            }
            ...
            window.decorateStoryView = function (view) {
                ...
                let run = [];
                children.forEach(el => {
                    // Konzeptkarte + Lern-Block werden eigene Sektionen
                    const standalone = el.tagName === 'ASIDE' && (el.classList.contains('insert-lernblock') || el.classList.contains('insert-wortdestages'));
                    if (standalone) {
