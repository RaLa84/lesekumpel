...
CSS_BLOCK = """
        ...
        /* ── Großbild-Skalierung (Beamer / 75-Zoll-Klassentafel) ──
           FLUID statt Stufen: Schrift wächst ab 1200px mit der Bildschirmbreite
           (Kinder sitzen im Klassenraum weit weg). Textspalten in ch -> die
           Zeilenlänge bleibt bei jeder Schriftgröße lesbar; Kästen/Einschübe
           skalieren über em automatisch mit. */
        @media (min-width: 1200px) {
            body:not(.view-tafel) .main-card { max-width: min(92vw, 2900px); }
            /* 1200px->~24px, 1920px->~36px, 2560px->~46px, 3840px->~67px */
            body:not(.view-tafel) .story-text { font-size: calc(0.25rem + 1.65vw); }
            body:not(.view-tafel) #full-text-view .mag-section { gap: 1.2em 2.5em; }
            body:not(.view-tafel) #full-text-view .mag-section > h3,
            body:not(.view-tafel) #full-text-view .mag-section > p { max-width: 34ch; }
            body:not(.view-tafel) #full-text-view .mag-section > aside.story-insert { max-width: 52ch; }
            body:not(.view-tafel) #full-text-view .mag-section.mag-intro > p,
            body:not(.view-tafel) #full-text-view .mag-section.mag-noimg > h3,
            body:not(.view-tafel) #full-text-view .mag-section.mag-noimg > p { max-width: 44ch; }
            body:not(.view-tafel) .hero-title-box #main-title { font-size: clamp(2rem, 0.6rem + 1.9vw, 5rem); }
            .mag-topic-pill { font-size: clamp(0.92rem, 0.4rem + 0.55vw, 1.8rem); }
            .mag-author-chip .ac-name { font-size: clamp(0.95rem, 0.45rem + 0.5vw, 1.7rem); }
            .mag-author-chip .ac-role { font-size: clamp(0.78rem, 0.4rem + 0.4vw, 1.35rem); }
            .mag-author-chip img { width: clamp(42px, 2.6vw, 80px); height: clamp(42px, 2.6vw, 80px); }
            body:not(.view-tafel) .reading-action-bar .rab-btn { min-height: clamp(54px, 3vw, 96px); min-width: clamp(62px, 3.4vw, 120px); }
            body:not(.view-tafel) .reading-action-bar .rab-icon { font-size: clamp(1.8rem, 1rem + 0.7vw, 3.2rem); }
            body:not(.view-tafel) .related-grid { grid-template-columns: repeat(auto-fit, minmax(150px, clamp(200px, 9vw, 340px))); }
            /* Tafel-Modus skaliert ebenso fluid */
            body.view-tafel .story-text { font-size: calc(0.4rem + 1.9vw); }
        }
